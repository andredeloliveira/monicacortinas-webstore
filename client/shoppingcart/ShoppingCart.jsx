MyCart = React.createClass({
                ...
                <div className="item" key={index}>
                  <div className="image">
                    ...
                    <div className="content">
                      ...
                      <div className="extra">
                        <div>
                          Quantidade: <input type="number" name={item.name} min="1" max="100" defaultValue={item.quantity} onChange={this.handleQuantityChange}/>
                        </div>
                        <button className="ui button" onClick={this.removeItem.bind(this, item, index)}><i className="icon remove"></i> Remover</button>
                      </div>
                    </div>
                  </div>
        );
  ...
  getTotal(){
    ...
    if(this.data.shoppingCart.items.length === 0){
      return <h1>Seu carrinho está vaio! <a href="/">Compre aqui para comprar!</a></h1>;
    }
            ...
            <div className="ui grid right aligned">
              <div className="row">
                <div className="column">
                  <button className="ui huge button" onClick={this.goToCheckout}>Finalizar</button>
                </div>
              </div>
            </div>
          </div>
